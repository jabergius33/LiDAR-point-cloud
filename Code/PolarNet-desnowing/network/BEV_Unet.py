...
class BEV_Unet(nn.Module):
    ...
    def __init__(self,n=19,n_height=32,dilation = 1,group_conv=False,input_batch_norm = False,dropout = 0.,circular_padding = False, dropblock = True, use_vis_fea=False):
        super(BEV_Unet, self).__init__()
        self.n = n
        self.n_height = n_height
        if use_vis_fea:
            self.network = UNet(19*n_height,2*n_height,dilation,group_conv,input_batch_norm,dropout,circular_padding,dropblock)
        else:
            self.network = UNet(19*n_height,n_height,dilation,group_conv,input_batch_norm,dropout,circular_padding,dropblock)
    ...
    def forward(self, x):
        ...
        new_shape = list(x.size())[:3] + [self.n_height,1] 
        x = x.view(new_shape) 
...
class UNet(nn.Module):
    def __init__(self, n,n_height,dilation,group_conv,input_batch_norm, dropout,circular_padding,dropblock):
